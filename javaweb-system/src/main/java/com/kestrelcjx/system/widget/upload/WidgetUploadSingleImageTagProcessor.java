...
public class WidgetUploadSingleImageTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        if (StringUtils.isEmpty(tagValue)) {
            ...
            model.add(modelFactory.createOpenElementTag(
                    String.format("img id=\"%s_show_id\" src=\"%s\" alt=\"点击查看大图\" width=\"%s\" height=\"%s\"",
                            tagName, tagValue, Integer.valueOf(itemSize[0]), Integer.valueOf(itemSize[1]))));
        }
        ...
        model.add(modelFactory.createOpenElementTag(
                String.format("button type=\"button\" class=\"layui-btn\" id=\"btnUploadImg_%s\"", tagName)));
