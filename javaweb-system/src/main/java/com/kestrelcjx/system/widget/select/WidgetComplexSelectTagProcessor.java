...
public class WidgetComplexSelectTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        // 这里是将字典的内容拼装成一个下拉框
        model.add(modelFactory.createOpenElementTag(String.format("select name=\"%s\" id=\"%s\" lay-search=\"\"",
                showName, showName)));
...
//                st.setString(1, "0");
                ...
                while (rs.next()) {
                    ...
                    while (rs2.next()) {
                        ...
                        if (Integer.valueOf(tagValue) == cId) {
                            model.add(modelFactory.createOpenElementTag(
                                    String.format("option value=\"%s\" selected", cId)));
                        } else {
                    ...
                    // 关闭连接
                    JDBCUtils.closeResource(conn2, st2, rs2);
            ...
            for (Map.Entry<Integer, String> item : map.entrySet()) {
                if (Integer.valueOf(tagValue) == item.getKey()) {
                    model.add(modelFactory.createOpenElementTag(String.format("option value='%s' %s",
                            item.getKey().toString(), "selected=''")));
                } else {
