...
public class WidgetDateSelectTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        // 这里是将字典的内容拼装成一个下拉框
        model.add(modelFactory.createOpenElementTag(String.format("input name=\"%s\" id=\"%s\" value=\"%s\"" +
                " lay-verify=\"%s\" placeholder=\"%s\" autocomplete=\"off\" class=\"layui-input date-icon\"" +
                " type=\"text\"", showName, showName, tagValue, format, showTips)));
        // 创建JS
