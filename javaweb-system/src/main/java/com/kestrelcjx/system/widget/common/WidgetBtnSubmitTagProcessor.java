...
public class WidgetBtnSubmitTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        if (CommonUtils.inArray("submit", map)) {
            stringBuffer.append(String.format("<button class=\"layui-btn\" lay-filter=\"submitForm\"" +
                    " lay-submit>%s</button>", map.get("submit")));
        }
        // 关闭按钮
        if (CommonUtils.inArray("close", map)) {
            stringBuffer.append(String.format("<button class=\"layui-btn layui-btn-primary\" type=\"button\"" +
                    " ew-event=\"closeDialog\">%s</button>", map.get("close")));
        }
        // 重置按钮
        if (CommonUtils.inArray("reset", map)) {
            stringBuffer.append(String.format("\t<button type=\"reset\" class=\"layui-btn" +
                    " layui-btn-normal\">%s</button>", map.get("reset")));
        }
