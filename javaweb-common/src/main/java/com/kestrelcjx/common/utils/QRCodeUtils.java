...
import com.kestrelcjx.common.config.QrImage;
...
/**
 * 二维码生成解析工具类
 * 说明: 二维码是一种编码形式，将内容encode得到二维码；将内容decode,得到数据内容。
 * https://www.jianshu.com/p/a046ff84226e
 */
public class QRCodeUtils {
    private static final Logger logger = LoggerFactory.getLogger(QRCodeUtils.class);
    /**
     * 嵌入的图片(被程序处理后)的高度(px)<br>
    ...
    public static void modifyBasicParamsValues(Integer embeddedImgDefaultWidth, Integer embeddedImgDefaultHeight,
                                               Integer frameWidth, Color frameWidthColor, Color qrCodeColor,
                                               Color qrCodeBackgroundColor, Color fillColor) {
        if (embeddedImgDefaultWidth != null) {
            QRCodeUtils.embeddedImgDefaultWidth = embeddedImgDefaultWidth;
        }
        if (embeddedImgDefaultHeight != null) {
            QRCodeUtils.embeddedImgDefaultHeight = embeddedImgDefaultHeight;
        }
        if (frameWidth != null) {
            QRCodeUtils.frameWidth = frameWidth;
        }
        if (frameWidthColor != null) {
            QRCodeUtils.frameWidthColor = frameWidthColor.getRGB();
        }
        if (qrCodeColor != null) {
            QRCodeUtils.qrCodeColor = qrCodeColor.getRGB();
        }
        if (qrCodeBackgroundColor != null) {
            QRCodeUtils.qrCodeBackgroundColor = qrCodeBackgroundColor.getRGB();
        }
        if (fillColor != null) {
            QRCodeUtils.fillColor = fillColor.getRGB();
        }
    }
    ...
    public static String encodeQrCodeWithEmbeddedImg(String content, int width, int height,
            ...
            throws IOException, WriterException {
        boolean result = ImageIO.write(genBufferedImageWithEmbeddedImg(
                content, width, height, embeddedImgPath), "jpg", getFile(destImagePath));
        logger.info(" generate Qr code file {} {}", destImagePath, result ? "success" : "fail");
        return destImagePath;
    }
    ...
    public static void encodeQrCodeWithEmbeddedImg(String content, int width, int height, String embeddedImgPath,
            ...
            throws IOException, WriterException {
        boolean result = ImageIO.write(genBufferedImageWithEmbeddedImg(
                content, width, height, embeddedImgPath), "jpg", outputStream);
        logger.info(" generate Qr code file to OutputStream {}", result ? "success" : "fail");
    }

    /**
     * 生成上方带内嵌图片、带文字的二维码<br>
    ...
    public static String encodeQrCodeWithEmbeddedImgAndFonts(QrImage param) throws IOException, WriterException {
        ...
        imageArrayFonts = imageWithFontsBufferedImage.getRGB(
                0, 0, qrCodeImageWidth, fontsImageHeight,
                imageArrayFonts, 0, qrCodeImageWidth);
        int[] imageArrayQr = new int[qrCodeImageWidth * qrCodeImageHeight];
        imageArrayQr = qrImage.getRGB(0, 0, qrCodeImageWidth,
                qrCodeImageHeight, imageArrayQr, 0, qrCodeImageWidth);
        // 生成新图片(在setsetRGB时，通过控制图像的startX与startY, 可达到不同的拼接效果)
        BufferedImage newBufferedImage = new BufferedImage(
                qrCodeImageWidth, qrCodeImageHeight + fontsImageHeight, BufferedImage.TYPE_INT_RGB);
        // 图片在上， 文字在下
        // 设置上半部分的RGB
        newBufferedImage.setRGB(0, 0, qrCodeImageWidth,
                qrCodeImageHeight, imageArrayQr, 0, qrCodeImageWidth);
        // 设置下半部分的RGB
    ...
    /**
     * 生成(带内嵌图片的)二维码图片信息BufferedImage<br>
     * 注:BufferedImage即为二维码图片的数据容器，可以利用其进一步生成二维码图片
     *
     * @param content           二维码的数据内容
     * @param width             二维码的宽(px)
     ...
     * @throws WriterException WriterException
     */
    private static BufferedImage genBufferedImageWithEmbeddedImg(
            String content, int width, int height, String embeddedImagePath)
    ...
    /**
     * 把传入的原始图像按高度(或宽度)进行缩小，生成符合要求的图标的BufferedImage信息<br>
        ...
        if (autoFill) {
            ...
            if (width == destImage.getWidth(null)) {
                ...
                graphic.drawImage(destImage, (width - destImage.getWidth(null)) / 2,
                        0, destImage.getWidth(null), destImage.getHeight(null),
                        Color.white, null);
            }
            graphic.dispose();
            destImage = image;
