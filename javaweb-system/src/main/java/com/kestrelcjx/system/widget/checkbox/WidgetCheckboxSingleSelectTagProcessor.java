package com.kestrelcjx.system.widget.checkbox;
        ...
        if (!StringUtils.isEmpty(tagSql)) {
            ...
            for (Map.Entry<Integer, String> item : map.entrySet()) {
                if (tagValue.contains(item.getKey().toString())) {
                    model.add(modelFactory.createOpenElementTag(String.format("input name=\"%s[%s]\"" +
                                    " lay-skin=\"primary\" title=\"%s\" checked=\"\" type=\"checkbox\"",
                            showName, item.getKey(), item.getValue())));
                } else {
                    model.add(modelFactory.createOpenElementTag(String.format("input name=\"%s[%s]\"" +
                                    " lay-skin=\"primary\" title=\"%s\" type=\"checkbox\"",
                            showName, item.getKey(), item.getValue())));
                }
            }
        }
