...
public class JPushUtils {
     ...
     * @return 0=推送失败，1=推送成功
     */
    public static int sendToRegistrationId(List<String> alias, String notification_title,
                                           String msg_title, String msg_content, String extrasparam) {
        JPushClient jPushClient = new JPushClient(MASTERSECRET, APPKER);
        int result = 0;
        try {
            PushPayload pushPayload = buildPushObject_all_alias_alertWithTitle(
     ...
     * @return 0=推送失败，1=推送成功
     */
    public static int sendToRegistrationId2(List<String> alias, String notification_title,
                                            String msg_title, String msg_content,
                                            String extrasparam, String time) {
        JPushClient jPushClient = new JPushClient(MASTERSECRET, APPKER);
        int result = 0;
        try {
     ...
     * @return 0=推送失败，1=推送成功
     */
    public static int sendToAllAndroid(String notification_title, String msg_title,
        ...
        return PushPayload.newBuilder()
                ...
                .setNotification(Notification.newBuilder()
                        ...
                        .addPlatformNotification(IosNotification.newBuilder()
                                ...
                                // 传sound.caf表示此推送以项目里面打包的sound.caf声音来提醒，
                                // 如果系统没有此音频则以系统默认声音提醒；
                                // 此字段如果传空字符串，iOS9及以上的系统是无声音提醒，以下的系统是默认声音
                                .setSound("sound.caf")
                                // 此字段为透传字段，不会显示在通知栏。
                                // 用户可以通过此字段来做一些定制需求，如特定的key传要指定跳转的页面（value）
                                .addExtra("url", extrasparam)
                                // 此项说明此推送是一个background推送
                                // 想了解background看：http://docs.jpush.io/client/ios_tutorials/#ios-7-background-remote-notification
                                // .setContentAvailable(true)
                                .build()
                        )
                        .build()
                )
                // Platform指定了哪些平台就会像指定平台中符合推送条件的设备进行推送jpush的自定义消息，
                // sdk默认不做任何处理，不会有通知提示。
                // 建议看文档http://docs.jpush.io/guideline/faq/的[通知与自定义消息有什么区别？]
                ...
                .setOptions(Options.newBuilder()
                        ...
                        .setSendno(1)
                        //此字段的值是用来指定本推送的离线保存时长
                        // 如果不传此字段则默认保存一天，最多指定保留十天，单位为秒
                        .setTimeToLive(86400)
                        .build()
                )
                .build();
    ...
    private static PushPayload buildPushObject_ios_all_alertWithTitle(String notification_title, String msg_title, String msg_content, String extrasparam) {
        return PushPayload.newBuilder()
                ...
                // 指定推送的接收对象，all代表所有人，
                // 也可以指定已经设置成功的tag或alias或该应应用客户端调用接口获取到的registration id
                .setAudience(Audience.all())
    ...
    private static PushPayload buildPushObject_all_alias_alertWithTitle(
            List<String> alias, String notification_title, String msg_title,
            String msg_content, String extrasparam) {
        return PushPayload.newBuilder()
                // 指定要推送的平台，all代表当前应用配置了的所有平台，也可以传android等具体平台
                .setPlatform(Platform.all())
                // 指定要推送的接受对象，all代表所有人
                // 也可以指定已经设置成功的tag或alias或该应用客户端调用接口获取到的registration id
                .setAudience(Audience.alias(alias))
                // jpush的通知，android的有jpush直接下发，IOS的由apns服务器下发，Winphone的由mpns下发
                .setNotification(Notification.newBuilder()
                        // 指定当前推送的android通知
                        .addPlatformNotification(AndroidNotification.newBuilder()
                                .setAlert(msg_content)
                        ...
                        .addPlatformNotification(IosNotification.newBuilder()
                                ...
                                .incrBadge(1)
                                // 此字段的值default表示系统默认声音；
                                // 传sound.caf表此推送以项目里面打包的sound.caf声音来提醒
                                // 如果系统没有此音频则以系统默认声音提醒；
                                // 此字段如果传空字符串，IOS9及以上的系统是无声音提醒，以下的系统是默认声音
                                .setSound("sound.caf")
                                // 此字段为透传字段，不会显示在通知栏。
                                ...
                                .addExtra("url", extrasparam)
                                // 此项说明此推送是一个background推送
                                // 想了解background，看：http://docs.jpush.io/client/ios_tutorials/#ios-7-background-remote-notification
                                // 取消此注释，消息推送时ios将无法在锁屏情况接受
                                // .setContentAvailable(true)
                                .build())
                        .build())
                // Platform指定了哪些平台就会向指定平台中符合推送条件的设备进行推送jpush的自定义消息
                // sdk默认不做任何处理，不会有通知提示。
                // 建议看文档：http://docs.jpush.io/guideline/faq/的[通知与自定义消息有什么区别？]
                .setMessage(Message.newBuilder()
                        .setMsgContent(msg_content)
                        .setTitle(msg_title)
                        .addExtra("url", extrasparam)
                ...
                .setOptions(Options.newBuilder()
                        ...
                        // 对android和自定义消息无意义
                        .setApnsProduction(true)
                        // 此字段是给开发者自己给推送编号，方便推送者分辨推送消息
                        .setSendno(1)
                        // 此字段的值是用来指定本推送的离线保存时长
                        // 如果不传此字段则默认保存一天，最多指定保存十天
                        .setTimeToLive(86400)
                        .build())
                .build();
    }
