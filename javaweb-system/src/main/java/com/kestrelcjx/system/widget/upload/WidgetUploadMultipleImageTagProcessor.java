package com.kestrelcjx.system.widget.upload;
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        for (String s : stringList) {
            ...
            model.add(modelFactory.createOpenElementTag(
                    String.format("div class=\"del_img\" onclick=\"remove_image_%s(this);\"", tagName)));
            ...
            model.add(modelFactory.createOpenElementTag(
                    String.format("img name=\"img_src_%s\" src=\"%s\" alt=\"%s(点击放大预览)\"" +
                                    " title=\"%s(点击放大预览)\" width=\"%s\" height=\"%s\"",
                            tagName, s, tagTitle, tagTitle, itemSize[0], itemSize[1])));
            model.add(modelFactory.createCloseElementTag("a"));
            model.add(modelFactory.createCloseElementTag("div"));
        }
        model.add(modelFactory.createOpenElementTag(
                String.format("div class=\"layui-upload-drag img_upload_%s\"", tagName)));
        model.add(modelFactory.createOpenElementTag(
                String.format("img id=\"btnUploadImg_%s\"" +
                                " src=\"/static/assets/images/default_upload.png\"" +
                                " alt=\"上传%s\" title=\"上传%s\" width=\"%s\" height=\"%s\"",
                        tagName, tagTitle, tagTitle, itemSize[0], itemSize[1])));
        model.add(modelFactory.createOpenElementTag(
                String.format("input type=\"hidden\" id=\"%s\" name=\"%s\" value=\"\"", tagName, tagName)));
        model.add(modelFactory.createCloseElementTag("div"));
        // 创建CSS
