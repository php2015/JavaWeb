package com.kestrelcjx.system.widget.button;
...
public class WidgetBtnFuncTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        if (tagType == 1) {
            // 大组件
            model.add(modelFactory.createOpenElementTag(String.format("a href=\"javascript:\" class=\"layui-btn" +
                            " btnOption %s layui-btn-small btn%s\" id=\"%s\" data-param='%s' lay-event=\"%s\"",
                    tagColor, tagIDName, tagID, param, tagID)));
        } else if (tagType == 2) {
            // 小组件
            model.add(modelFactory.createOpenElementTag(String.format("a href=\"javascript:\"" +
                    " class=\"layui-btn btnOption %s layui-btn-xs btn%s\" id=\"%s\" data-param='%s'" +
                    " lay-event=\"%s\"", tagColor, tagIDName, tagID, param, tagID)));
        }
