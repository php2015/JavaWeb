package com.kestrelcjx.system.widget.select;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.kestrelcjx.common.utils.JDBCUtils;
import com.kestrelcjx.common.utils.StringUtils;
import com.kestrelcjx.common.utils.TreeUtils;
...
public class WidgetTreeSelectTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag,
        ...
        // 这里是将字典的内容拼装成一个下拉框
        model.add(modelFactory.createOpenElementTag(String.format("select name='%s' id='%s' lay-verify='%s'" +
                " lay-search='' lay-filter='%s'", showName, showName, isRequire, showName)));
        ...
        if (!StringUtils.isEmpty(tagSql)) {
            ...
            try {
                ...
                // 创建树结构
                JSONArray result = TreeUtils.listToTree(JSONArray.parseArray(JSON.toJSONString(mapList)),
                        "id", "pid", "children");
                // 创建构造器
                for (Object item : result) {
                    ...
                    List<Map<String, Object>> children =
                            (List<Map<String, Object>>) ((JSONObject) item).get("children");
                    if (children != null) {
            ...
            for (Map.Entry<Object, String> item : map.entrySet()) {
                if (tagValue.equals(item.getKey().toString())) {
                    model.add(modelFactory.createOpenElementTag(
                            String.format("option value='%s' %s", item.getKey().toString(), "selected=''")));
                } else {
                    model.add(modelFactory.createOpenElementTag(
                            String.format("option value='%s'", item.getKey().toString())));
                }
